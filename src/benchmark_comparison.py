...
from benchmark import format_system_info, get_system_info, measure_query_latency
...
def run_comparison(
    ...
    results = {
        ...
        "config": {"k": k, "num_trials": num_trials, "num_queries": len(queries)},
        "indexes": [],
        "comparisons": [],
    }
...
def format_comparison_table(results: dict[str, Any]) -> str:
    ...
    lines.append("=" * 100)
    lines.append(
        f"{'Index':<25} {'Method':<20} {'Avg Latency (ms)':<18} {'P95 (ms)':<12} {'Size (MB)':<12}"
    )
    lines.append("-" * 100)
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Compare performance across different retrieval configurations"
    )
    parser.add_argument(
        "--output", default="outputs/benchmark_comparison.json", help="Output file for results"
    )
    parser.add_argument("--num-trials", type=int, default=20, help="Number of trials per query")
    ...
    index_configs = [
        {
            ...
            "methods": [{"name": "tfidf", "method": "tfidf"}],
        },
        {
            "name": "Chunked (TF-IDF)",
    ...
    print(
        f"Running benchmark comparison with {len(queries)} queries, {args.num_trials} trials each"
    )
    print("=" * 100)
    ...
    if results["indexes"]:
        ...
        all_methods = [
            (idx["name"], method) for idx in results["indexes"] for method in idx["methods"]
        ]
        ...
        print(
            f"Fastest: {fastest[0]} / {fastest[1]['name']} - {fastest[1]['avg_mean_latency_ms']:.2f}ms avg"
        )
        print(
            f"Slowest: {slowest[0]} / {slowest[1]['name']} - {slowest[1]['avg_mean_latency_ms']:.2f}ms avg"
        )
        ...
        if fastest[1]["avg_mean_latency_ms"] > 0:
            ...
            print("Speed difference: >1000x (fastest method < 0.01ms)")
