...
from rag import GroundedResult, retrieve_grounded
...
def format_grounded_answer(query: str, results: list[GroundedResult]) -> str:
    ...
    for result in results:
        answer_parts.append(
            f"  {result.citation} - Document {result.source_doc_id}, chars {result.char_range}"
        )
...
def main() -> None:
    ...
    parser.add_argument(
        "--index", default="outputs/index_chunked.pkl", help="Path to chunked index"
    )
    parser.add_argument(
        "--queries",
        nargs="+",
        default=["What is reinforcement learning?", "How do you evaluate retrieval?"],
        help="Queries to run",
    )
    parser.add_argument("--k", type=int, default=3, help="Number of results")
    parser.add_argument(
        "--method", default="hybrid", choices=["tfidf", "bm25", "embeddings", "hybrid"]
    )
    parser.add_argument(
        "--output", default="outputs/grounded_demo.txt", help="Output file for results"
    )
    ...
    for i, query in enumerate(args.queries, 1):
        ...
        results = retrieve_grounded(index, query, k=args.k, method=args.method)
