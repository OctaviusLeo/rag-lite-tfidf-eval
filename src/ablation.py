...
from rag import retrieve_hybrid, retrieve
        ...
        for line in f:
            ...
            if method == "tfidf_legacy":
                ...
                hits = retrieve_hybrid(
                    index, query, k=k, method=method, rerank=rerank, **kwargs
                )
...
def main() -> None:
    ...
    if not os.path.exists(args.index):
        ...
        print("  python src/build_index.py --bm25 --embeddings --reranker --out outputs/index_hybrid.pkl")
        return
    ...
    if index.bm25 is not None or index.embedder is not None:
        ...
        result = evaluate_method(
            index, args.eval, args.k, "hybrid",
            tfidf_weight=0.4, bm25_weight=0.3, embedding_weight=0.3
        )
        results.append(result)
        print(f"  Recall@{args.k}: {result['recall@k']:.4f} | MRR@{args.k}: {result['mrr@k']:.4f}")
    else:
